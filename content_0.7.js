...
(function () {
        ...
        requestAnimationFrame(() => {
            setTimeout(() => {
                ...
                .then(function (dataUrl) {
                    console.log('Screenshot successful, dataUrl length:', dataUrl.length);
    ...
    function addShareButton(actions) {
        ...
        button.addEventListener('click', async function (event) {  // 添加event参数以便后续使用
            ...
            const postLink = element.querySelector("a.post-date") || element.querySelector(".post-date a") || element.querySelector(".widget-link") || element.querySelector('a[href*="/t/topic/"]');
            const postHref = postLink?.getAttribute("href") || postLink?.href || window.location.pathname;
            const url = "https://shuiyuan.sjtu.edu.cn" + postHref.split("?")[0];
            const content = element.querySelector(".cooked").cloneNode(true);
            // 修复：兼容新版avatar结构
            const avatarImg = element.querySelector(".trigger-user-card.main-avatar img") || element.querySelector(".trigger-user-card.main-avatar")?.firstChild;
            const avatar = avatarImg?.getAttribute('src') || avatarImg?.src || '';
            const avatarBase64 = await imageUrlToBase64(avatar); // 如果转换失败，使用原URL

            // 修复：兼容新版用户名结构
            const namesContainer = element.querySelector(".topic-meta-data .names") || element.querySelector(".topic-meta-data");
            const fullname = namesContainer?.querySelector(".first")?.firstChild?.textContent || namesContainer?.querySelector('[class*="full-name"]')?.textContent || '';
            const username = namesContainer?.querySelector(".second")?.firstChild?.textContent || namesContainer?.querySelector('[class*="username"]')?.textContent || '';
            // 修复：添加容错处理
            const relativeDateEl = element.querySelector(".relative-date") || element.querySelector('[class*="relative-date"]') || element.querySelector('time');
            const time = relativeDateEl?.title || relativeDateEl?.getAttribute('datetime') || '';
...
(function(){
    ...
    function addPostNumberDisplay(element) {
        ...
        const postLink = element.querySelector(".post-info.arrow") || element.querySelector("a.post-date") || element.querySelector(".post-date a") || element.querySelector(".widget-link.post-date") || element.querySelector('a[href*="/t/topic/"]');
        if (!postLink) return;
    ...
    const observer = new MutationObserver((mutationsList) => {
        for (let mutation of mutationsList) {
            if (mutation.type === 'childList') {
                mutation.addedNodes.forEach((node) => {
                    if (node.nodeType === Node.ELEMENT_NODE && node.matches('.row') && !node.matches('.post-number-display')) {
                        // console.log("node is", node.cloneNode(true))
