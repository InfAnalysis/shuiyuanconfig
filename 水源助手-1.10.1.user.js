    ...
    const ALL_FEATURES = [
        ...
        },
        {
            ...
            onMatch: (element) => {
                ...
                const getStartYMD = () => {
                    const article = element.closest('article');
                    const dateSpan = article?.querySelector('a.post-date > span') || article?.querySelector('.post-date span') || article?.querySelector('time');
                    const dataTime = dateSpan?.getAttribute('data-time') || dateSpan?.getAttribute('datetime');
                    if (!dataTime) return null;
                    const postDate = new Date(parseInt(dataTime, 10) || Date.parse(dataTime));
                ...
                const updateDurationNode = (durationNode) => {
                    ...
                    durationDaysSpan.textContent = `（${getDayDifference(startYMD, endYMD)} 天）`;
